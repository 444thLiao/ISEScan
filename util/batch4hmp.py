...
def batch(args):
    ...
    dir2proteome4org = os.path.join(dir4prediction,'proteome')
    dir2hmmsearchResults = os.path.join(dir4prediction,'hmm')
    cmdargs = ' '.join([python3, cmd, '{infile}', dir2proteome4org, dir2hmmsearchResults, "-odir", dir4prediction])
    # summarize IS elements in each genome DNA and each organism
    for org in file4orgs.keys():
        files = file4orgs[org]
        for file in files:
            callcmd = cmdargs.format(infile=file)
